...
public class CS401BST<E extends Comparable<E>> 
{
    ...
    public boolean add (E element) {
        ...
        else root = recAdd(element, root); //Calling the recursive function
        size++;
        return true;
    }
    
    // This method takes the element and add it to the existing tree root recursively
    public TreeNode<E> recAdd(E element, TreeNode<E> root) {
    	
    	if(root == null )
    		root = new TreeNode<E> (element, null);
        else if( ((Comparable) element).compareTo( root.element ) < 0 ){
        	if(root.left == null)
        		root.left = new TreeNode<E>(element, root);
        	else
        		root.left = recAdd(element, root.left);
        }
        else if( ((Comparable) element).compareTo( root.element ) > 0 ){
        	if(root.right == null)
        		root.right = new TreeNode<E>(element,root);
        	else
        		root.right = recAdd(element, root.right );
        }
        else
            ;
        return root;	
    ...
    /**
     * This method is used to calculate height of a BST
     * @param aNode Node of which height needs to be calculated
     * @return height of the node
     */
    protected int h(TreeNode<E> node)
    {
        if(node == null)
            return -1;
